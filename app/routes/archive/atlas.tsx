...
export default function Atlas({ loaderData }: Route.ComponentProps) {
	...
	return (
		<div className="register-void relative h-[calc(100vh-4rem)] w-full overflow-hidden">
			<AtlasCanvas
				hits={hits}
				colorMode={colorMode}
			...
			<div className="pointer-events-none absolute inset-0 flex flex-col justify-between p-6">
				<div className="pointer-events-auto flex flex-wrap items-start gap-6">
					...
					<div className="bg-ground/80 max-w-md p-3">
						...
						<p className="text-ground-muted mt-1 max-w-sm font-mono text-[11px] leading-relaxed">
							{manifest
								? `${manifest.count.toLocaleString()} readings of ${manifest.works.toLocaleString()} works, projected from 1,024 dimensions to 3 with UMAP. The layout is fixed; a search lights it.`
						...
						<p className="text-ground-muted mt-2 max-w-sm font-mono text-[11px] leading-relaxed">
						...
						<Form method="get" className="mt-4 flex items-center gap-2">
							<input
								...
								className="border-rule text-ground-fg placeholder:text-ground-muted focus:border-link w-72 border bg-transparent px-3 py-2 font-mono text-xs focus:outline-none"
							/>
							<button
								type="submit"
								className="border-rule text-ground-fg hover:border-link hover:text-link border px-3 py-2 font-mono text-xs"
							>
							...
							{query ? (
								<Link
									to="/archive/atlas"
									className="text-ground-muted hover:text-link font-mono text-xs underline underline-offset-4"
								>
									clear
								</Link>
							) : null}
				...
				<div className="flex items-end justify-between gap-6">
					...
					{/* Kept clear of the bottom-right corner, where the theme control
					    and the dev indicator sit. */}
					<p className="bg-ground/80 text-ground-muted mr-16 max-w-60 p-3 text-right font-mono text-[10px] leading-relaxed">
			...
			{hover ? (
				<div
					className="border-rule bg-ground/95 pointer-events-none absolute z-10 max-w-xs border px-3 py-2"
					style={{
...
function Legend({
	...
	return (
		<div className="bg-ground/80 text-ground-muted flex flex-col gap-1.5 p-3 font-mono text-[10px]">
			{hasQuery ? (
			...
			{colorMode === 'spread' ? (
				<span className="flex flex-col gap-1">
					<span className="flex items-center gap-2">
						...
						{manifest ? manifest.spreadCeiling.toFixed(2) : '—'} (the 1st to
						99th percentile; measured in 1,024 dimensions, not in this picture)
					</span>
					<span className="flex items-center gap-2">
						<span
							className="inline-block h-2 w-2 rounded-full"
